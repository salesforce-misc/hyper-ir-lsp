...
use dashmap::DashMap;
use hir_language_server::chumsky::{
    parse, type_inference, Func, ImCompleteSemanticToken, ParserResult,
};
use hir_language_server::completion::completion;
use hir_language_server::jump_definition::get_definition;
use hir_language_server::reference::get_reference;
use hir_language_server::semantic_token::{semantic_token_from_ast, LEGEND_TYPE};
use ropey::Rope;
        ...
        let inlay_hint_list = hashmap
            ...
            .map(|(k, v)| {
                (
                    ...
                    k.end,
                    match v {
                        hir_language_server::chumsky::Value::Null => "null".to_string(),
                        hir_language_server::chumsky::Value::Bool(_) => "bool".to_string(),
                        hir_language_server::chumsky::Value::Num(_) => "number".to_string(),
                        hir_language_server::chumsky::Value::Str(_) => "string".to_string(),
                        hir_language_server::chumsky::Value::List(_) => "[]".to_string(),
                        hir_language_server::chumsky::Value::Func(_) => v.to_string(),
                    },
                )
            })
            .filter_map(|item| {
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let completions = || -> Option<Vec<CompletionItem>> {
            ...
            for (_, item) in completions {
                match item {
                    hir_language_server::completion::ImCompleteCompletionItem::Variable(var) => {
                    ...
                    hir_language_server::completion::ImCompleteCompletionItem::Function(
                        name,
                        args,
                    ) => {
                        ret.push(CompletionItem {
